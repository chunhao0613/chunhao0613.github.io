...
import { ArrowUpRight, Github, Linkedin, Mail, Terminal, BookOpen, ChevronRight, Menu, X, Activity, Sun, Moon, Code } from 'lucide-react';
...
// --- 組件: 多方向漸顯容器 ---
const Reveal = ({ children, delay = 0, className = '', type = 'fade-up', triggerOnMount = false }) => {
  ...
  return (
    <div className={`min-h-screen bg-zinc-950 text-zinc-200 font-sans selection:bg-zinc-800 selection:text-white relative overflow-hidden transition-colors duration-500 ${isDarkMode ? '' : 'theme-light'}`}>
      ...
      <button
        ...
        onMouseEnter={(e) => { handleMouseEnter(e); setIsSidebarOpen(true); }}
        onMouseLeave={handleMouseLeave}
        className={`fixed top-6 left-6 z-40 p-3 bg-zinc-900/50 border border-zinc-800 rounded-full text-zinc-400 hover:text-white hover:bg-zinc-700 transition-all backdrop-blur-md ${isSidebarOpen ? 'opacity-0 scale-90 pointer-events-none' : 'opacity-100 hover:scale-110 shadow-lg'}`}
      >
        <Menu size={20} />
      </button>
      ...
      {/* 側邊欄 */}
      <div
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-40 transition-opacity duration-500 ${isSidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsSidebarOpen(false)}
      />
      <div
        onMouseLeave={() => setIsSidebarOpen(false)}
        className={`fixed top-0 left-0 h-full w-full sm:w-80 bg-zinc-950 border-r border-zinc-800 z-50 transform transition-transform duration-500 ease-[cubic-bezier(0.19,1,0.22,1)] flex flex-col p-8 ${isSidebarOpen ? 'translate-x-0 shadow-[20px_0_50px_rgba(0,0,0,0.5)]' : '-translate-x-full'}`}
      >
        <div className="flex justify-between items-center mb-16">
          <span className="font-mono text-sm text-zinc-500 border border-zinc-800 px-3 py-1 rounded-full tracking-widest">MENU</span>
          <button onClick={() => setIsSidebarOpen(false)} className="p-2 text-zinc-500 hover:text-white bg-zinc-900 hover:bg-zinc-800 rounded-full transition-all">
            <X size={20} />
          </button>
        </div>
        <nav className="flex flex-col gap-8">
          {[
            ...
            >
              <span className="font-mono text-sm text-zinc-600 group-hover:text-zinc-400 transition-colors">{item.id}</span>
      ...
      {clickRipples.map(ripple => (
        <div key={ripple.id} className="pointer-events-none fixed z-40 rounded-full border border-white/40 animate-ripple-click" style={{ left: ripple.x - 20, top: ripple.y - 20, width: '40px', height: '40px' }} />
      ))}
      <div className="fixed top-0 left-0 h-1 bg-gradient-to-r from-zinc-500 to-zinc-100 z-50 transition-all duration-150 ease-out shadow-[0_0_10px_rgba(255,255,255,0.5)]" style={{ width: `${scrollProgress * 100}%` }} />
      <div className="pointer-events-none fixed inset-0 z-0 transition-opacity duration-300" style={{ background: `radial-gradient(600px circle at ${mousePos.x}px ${mousePos.y}px, ${isDarkMode ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)'}, transparent 40%)` }} />
      <div className={`pointer-events-none fixed z-50 rounded-full border border-white/50 mix-blend-difference transition-all duration-300 ease-out hidden md:flex items-center justify-center ${isHovering ? 'w-16 h-16 bg-white/20' : 'w-8 h-8 bg-transparent'}`} style={{ transform: `translate(${mousePos.x - (isHovering ? 32 : 16)}px, ${mousePos.y - (isHovering ? 32 : 16)}px)` }}>
        <div className={`w-1 h-1 bg-white rounded-full transition-opacity duration-300 ${isHovering ? 'opacity-100' : 'opacity-0'}`} />
      </div>
            ...
            <div>
              ...
              <Reveal delay={100} type="fade-up" triggerOnMount={true}>
                <h1 className="text-5xl md:text-7xl font-extrabold tracking-tighter leading-tight mb-4">
                  Hi, 我是<br />余駿豪 <span className="text-3xl md:text-5xl text-zinc-500 font-medium tracking-normal">/ 鮭魚</span>
                </h1>
              ...
              <Reveal delay={200} type="fade-up" triggerOnMount={true}>
                <p className="text-lg text-zinc-400 leading-relaxed mb-10 max-w-lg">
                  我是一名對開發充滿好奇心與創新思維的軟體人。擅長將複雜的技術概念轉化為易懂的架構與交接文件，包含嚴謹的 SRS 與系統分析。具備多元的技術棧，從建構 AI RAG 應用到區塊鏈智慧家居，熱衷於將各種點子落地解決實際問題。
                </p>
              </Reveal>
              <Reveal delay={300} type="fade-up" triggerOnMount={true}>
                <div className="flex gap-4 items-center">
                  <a href="#projects" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="bg-white text-black px-6 py-3 rounded-full font-bold hover:scale-105 transition-transform flex items-center gap-2">
                    查看個人專案 <ArrowUpRight size={18} />
                  </a>
                  <a href="#contact" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="text-zinc-400 hover:text-white px-6 py-3 transition-colors font-medium">
                    聯絡我
                  </a>
                </div>
              </Reveal>
        ...
        <section id="projects" className="py-32 border-b border-zinc-900">
          <Reveal>
            <div className="flex items-center gap-3 mb-20">
              ...
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight">個人專案</h2>
            </div>
          </Reveal>
                  ...
                  <div className={`flex flex-col justify-center ${idx % 2 !== 0 ? 'md:order-1' : ''}`}>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {project.tags.map(tag => (
                        <span key={tag} className="text-xs px-4 py-2 bg-zinc-900 border border-zinc-800 rounded-full text-zinc-300 font-mono">
                          {tag}
                        </span>
                      ))}
                    </div>
